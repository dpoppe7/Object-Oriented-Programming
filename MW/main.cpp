...
gameParameters readGameParams(IO io){
    ...
    //std::cout << io.gameParams.row << io.gameParams.col << io.gameParams.mines << std::endl;
...
std::tuple<bool, bool, bool, Minesweeper::Board> playGame(IO io , Game game){
    bool isOver = false;
    bool userQuit = false;
    bool userWon = false;
    char sign, c;
    ...
    game.setTotalCells(game.gameBoard);
    ...
    userWon = game.ifUserWon();

    return std::make_tuple(isOver, userQuit, userWon, game.gameBoard);
}
...
int main(){
    ...
    //playGame
    std::tuple<bool, bool, bool, Minesweeper::Board> gameOver = playGame(io, game);
    bool if_isOver = std::get<0>(gameOver);   //true if Game Over
    bool if_quit = std::get<1>(gameOver);     //true if user Quit
    bool if_userWon = std::get<2>(gameOver);  //true if user Won
    game.gameBoard = std::get<3>(gameOver);   //current game board
    ...
    if (if_isOver){
        ...
        io.reportGameResults(if_quit, if_userWon);
